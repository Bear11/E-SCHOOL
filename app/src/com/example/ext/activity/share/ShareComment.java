...
public class ShareComment extends Activity {
	...
	private Handler operate = new Handler() {
		...
		public void handleMessage(Message msg) {
			if ("error".equals(msg)) {
				Toast.makeText(ShareComment.this, "评论失败，请重试",
						Toast.LENGTH_SHORT).show();
				/*Intent it = new Intent(ShareComment.this, SharingCenterDetail.class); // 实例化Intent
				it.putExtra("vaule","error");	
				startActivity(it);*/
			} else {
				Toast.makeText(ShareComment.this, "评论成功", Toast.LENGTH_SHORT)
						.show();
				/*Intent it = new Intent(ShareComment.this, SharingCenterDetail.class); // 实例化Intent
	...
	protected String initData() {
		...
		httpUrl = GlobalFinal.path
				.concat("sharefc_savaShareFirstCommnet.action?");
		...
		final String shareId = it.getStringExtra("sid"); // 取得设置的分享表id
